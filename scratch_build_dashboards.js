const fs = require('fs');
const pages = ['reports', 'requests', 'supplies'];
const titles = ['Reports', 'Requests', 'Supplies'];

pages.forEach((page, index) => {
    const dir = `src/app/dashboard/super-admin/${page}`;
...
export default function ${titles[index]}Dashboard() {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-950 mt-4">Manage ${titles[index]}</h1>
          <p className="text-sm text-slate-500 mt-2">Overview of all ${page} records on the platform.</p>
        </div>
      </div>
      ...
      <div className="bg-white border text-sm border-slate-200 rounded-3xl overflow-hidden shadow-sm">
        {loading ? (
           ...
           <div className="p-10 text-center text-slate-500">No ${page} found in database.</div>
        ) : (
